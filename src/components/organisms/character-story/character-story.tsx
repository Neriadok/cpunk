...
import { baseAge, getRandomLifePath } from '../../../lib/character-generator';
...
function CharacterStory({ subject, character, readonly }: CharacterStoryProps) {
  return (
    <Box>
      ...
      <Stack>
        {character.events.map(
          ({ type, facts }: LifePathEvent, index: number) => (
            <Stack key={`event${index}`}>
              <Divider />
              <Typography
                flex="1"
                variant="body1"
                ...
                color="inherit"
              >
                {t(`life-path.event-age`, { age: index + baseAge - 2 }) +
                  ' ' +
                  t(`life-path.event-${type}-label`)}
              </Typography>
              <Stack>
                {facts.map((fact, j) => (
                  <Typography
                    key={`event${index}-fact${j}`}
                    flex="1"
                    variant="body2"
                    component="div"
                    color="text.secondary"
  ...
  function resetStory() {
    ...
    const baseCharacter = {
      ...character,
      skills: getAverageRoleSkills(character.role),
      ...getRandomLifePath(character.age, character.gender),
    };
    subject?.next({
      ...baseCharacter,
