...
function CharacterRoleSkills({
  ...
  return (
    <Box>
      <Stack direction="row" sx={{ display: 'flex' }}>
        ...
        >
          {t('character.roleskills')}
        </Typography>
        <Typography variant="h5" component="div" color={color}>
          {roleSkillPoints}/{maxRoleSkillPoints}
        </Typography>
      </Stack>
      <Stack>
        {roleSkills.map(({ stat, skill }) =>
          stat === 'special'
            ? getSpecialRoleSkill(skill)
            : getRoleSkill(stat, skill)
        )}
      </Stack>
    </Box>
  );
  ...
  function getRoleSkill(stat: Stat, skill: Skill) {
    return (
      ...
      >
        <Typography width="120px" variant="body2" color="text.secondary">
        ...
        <Box sx={{ flex: 1, pr: 1 }}>
          <Slider
            ...
            max={10}
            onChange={(e) => setRoleSkill(e, skill)}
          />
        </Box>
      </Stack>
    );
  ...
  function getSpecialRoleSkill(skill: Skill) {
    return (
      <Stack
        ...
        sx={{ display: 'flex', alignItems: 'center' }}
      >
        <Typography width="120px" variant="body2" color="inherit">
          ({getSpecialSkillMoney(character)}$) {t('character.skill.' + skill)}
        </Typography>
        <Box sx={{ flex: 1, pr: 1 }}>
          <Slider
            valueLabelDisplay="auto"
